...
from SpiderContext import Schema
...
def update_grammar_with_tables(grammar_dictionary: Dict[str, List[str]],
    ...
    table_names = sorted([f'"{table.lower()}"' for table in
                          schema.table_names], reverse=True)
    grammar_dictionary['table_name'] += table_names

    all_columns = set()
    for i, table in enumerate(schema.table_names_original):
        all_columns.update([f'"{table.lower()}@{col[1].lower()}"' for col in schema.column_names_original if col[0] == i])
